...
async def init_db():
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        base_prices = [
            ('general_cleaning', 150.0, 'Генеральная уборка (цена за м²)', 'base'),
            ('after_repair', 250.0, 'Уборка после ремонта (цена за м²)', 'base'),
            ('support_cleaning', 100.0, 'Поддерживающая уборка (цена за м²)', 'base'),
        ]
        
        # 3. Дополнительные услуги (фиксированная цена)
        addon_prices = [
            ('windows_price', 300.0, 'Мытье окон (цена за м²)', 'addon'),
